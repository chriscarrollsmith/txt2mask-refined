...
class Script(scripts.Script):
    def title(self):
        return "txt2mask v0.1.1 - Refined"
    ...
    def ui(self, is_img2img):
        ...
        smoothing = gr.Slider(label="Smoothing", minimum=0.0, maximum=100.0, step=1.0, value=20.0) #added smoothing
        brush_mask_mode = gr.Radio(label="Brush mask mode", choices=['discard','add','subtract'], value='discard', type="index", visible=False)
        smoothing_enabled = gr.Checkbox(label="Enable Smoothing?",value=True)
        ...
        return [mask_prompt,negative_mask_prompt, mask_precision, mask_padding, smoothing, brush_mask_mode, smoothing_enabled, mask_output, plug]
 
    def run(self, p, mask_prompt, negative_mask_prompt, mask_precision, mask_padding, smoothing, brush_mask_mode, smoothing_enabled, mask_output, plug):
